...
class PrediksiController extends Controller
{
    ...
    // mengambil data dari database
		$data = DB::table('faktor_kekerasan_terhadap_perempuan')->latest()->limit(19)->orderBy('id')->get();
		$ret['data'] = $data;

		// $data2 = DB::table('peta_kecamatan')->join('hasil_prediksi', 'hasil_prediksi.id_kecamatan', '=','peta_kecamatan.id_kecamatan')->latest()->limit(19)->orderBy('id_hasil')->get();
		
		$data2 = DB::table('hasil_prediksi')->join('faktor_kekerasan_terhadap_perempuan', 'hasil_prediksi.id', '=','faktor_kekerasan_terhadap_perempuan.id')->join('peta_kecamatan', 'faktor_kekerasan_terhadap_perempuan.id_kecamatan', '=','peta_kecamatan.id_kecamatan')->limit(19)->orderBy('id_hasil','desc')->get();

		$ret['dataPred'] = $data2;

		// mengambil data hasil prediksi
		$hasil_dari_prediksi = array();
		for ($i=0; $i<count($data2); $i++) {
			$hasil_dari_prediksi[$i]=$data2[$i]->hasil_dari_prediksi;
		}

		// average hasil prediksi
		$average = array_sum($hasil_dari_prediksi) / count($hasil_dari_prediksi);

		// standar deviation hasil prediksi
		$num_of_elements = count($hasil_dari_prediksi);
          
        $variance = 0.0;
          
        // calculating mean using array_sum() method
        $average = array_sum($hasil_dari_prediksi)/$num_of_elements;
          
        foreach($hasil_dari_prediksi as $i)
        {
		...
		// membuat batasan untuk hasil prediksi
		$batas_bawah = round($average-(1)*$stdev);
		$batas_atas = round($average+(1)*$stdev);
		// dd($batas_atas);
 		
		$ret['batas_bawah'] = $batas_bawah;
		$ret['batas_atas'] = $batas_atas;
    ...
    public function import(Request $request) 
    {
		...
		foreach ($array as $type) {
 			...
 			foreach ($array as $data){
	 			if($data[$i][0] !== null){
		 			$dataSet[]=[
		 				...
		 				'kasus_kekerasan' => $data[$i][2],
		 				'perempuan_bekerja' => $data[$i][3],
		 				'ibu_rumah_tangga' =>$data[$i][4],
		 				'pernikahan_dini' =>$data[$i][5],
		  			];
				}
 			}
 		}
 		...
 		// dd($dataSet);
 		DB::table('faktor_kekerasan_terhadap_perempuan')->insert($dataSet);

		// notifikasi dengan session
		Session::flash('sukses','Data Faktor Berhasil Diimport!');
		Session::flash('error','Data Faktor Gagal Diimport!');
    ...
    {
    	$data = DB::table('peta_kecamatan')->join('faktor_kekerasan_terhadap_perempuan', 'faktor_kekerasan_terhadap_perempuan.id_kecamatan', '=','peta_kecamatan.id_kecamatan')->latest()->limit(19)->orderBy('id')->get();
	...
	// mengambil data banyaknya kasus
		$variabelY = array();
		for ($i=0; $i<count($data); $i++) {
			$variabelY[$i]=$data[$i]->kasus_kekerasan;
		}
		// dd($variabelY);
    ...
    // Perhitungan model untuk Prediksi
		...
		for ($i=0; $i<sizeof($bobot); $i++) {
			$temp = 0;
			for ($j=0; $j<count($variabelY); $j++) {
				 $temp += $bobot[$i][$j] * $variabelY[$j];
			}
			$hasil[$i][$j]= $temp;
	...
	// menambahkan data ketabel Hasil Prediksi
		for ($i = 0; $i < count($data); $i++) {
	        $hasilPrediksi[] = [
	            ...
	            'hasil_dari_prediksi'=> $hasil[$i][19],
